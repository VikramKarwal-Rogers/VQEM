...
class VQEM_SESSION:
    ...
    def __vqem_score__(self, raw_df):
        ...
        ((self.PTBTP_weight * col("normalized_weighted_average_PTBTP")) + (self.TTTP_weight * col("normalized_tttp")) +
         (self.BRS_weight * col("normalized_weighted_average_bitrate")) + (self.STP_weight * col("normalized_stp"))))))).\
    ...
    def __average_by_session__(self,raw_df):
        ...
        return raw_df.groupBy("accountSourceId", "deviceSourceId", "pluginSessionId").avg("normalized_weighted_average_PTBTP",
    ...
    def __initial_method__(self, run_date):
        ...
        self.spark.sql("INSERT INTO default.vqem_session_score_level_staging_historical SELECT * from default.vqem_score_session_level_staging_detail")

        # combined_vqem_base = vqem_on_session_level.withColumn("event_date", substring(lit(run_date), 1, 10)). \
        ...
        #
        # combined_with_whix = self.obj.join_two_frames(combined_vqem_base, whix, "inner", ["deviceSourceId",
        #                                                                                   "hour"]). \
