...
class BRS:
    ...
    def __normalized_weighted_average_bitrate__(self,raw_df):
        ...
        raw_df = raw_df.withColumn("weighted_average_bitrate",
                                       func.when(col("weighted_average_bitrate") <= min_bound, min_bound). \
                                       otherwise(func.when(col("weighted_average_bitrate")>=max_bound, max_bound).otherwise(col("weighted_average_bitrate"))))

        return raw_df.withColumn("normalized_weighted_average_bitrate",
                                 (((col("weighted_average_bitrate") - min_bound) / ((max_bound - min_bound)))))
